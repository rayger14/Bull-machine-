...
def run_single_test(bars_confirm, drop_pct, bars_max, test_num):
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "--out", f"exit_sweep_results_{run_id}"
...
def main():
    ...
    print("-" * 85)
    print(f"{'Rank':<4} {'Confirm':<7} {'Drop%':<7} {'MaxBars':<8} {'Trades':<6} {'WinRate':<8} {'Expectancy':<10} {'MaxDD':<8}")
    print("-" * 85)

    for i, result in enumerate(successful_tests[:5]):
        print(f"{i+1:<4} {result['bars_confirm']:<7} {result['drop_pct']:<7.2f} {result['bars_max']:<8} "
