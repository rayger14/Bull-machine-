#!/usr/bin/env python3
"""
Exit Parameter Sweep - Final Version

Tests different exit parameter combinations to optimize exit effectiveness.
Uses backtest-level configuration for exit signals (not strategy-level).
"""
...
def run_single_test(bars_confirm, drop_pct, bars_max, test_num):
    ...
    try:
        ...
        backtest_config = {
            ...
            # EXIT PARAMETERS BELONG HERE (backtest level), NOT in strategy config
            "exit_signals": {
                "enabled": True,
                ...
                "choch_against": {
                    "swing_lookback": 3,
                    "bars_confirm": bars_confirm,  # PARAMETER SWEEP
                    "min_break_strength": 0.05
                },
                "momentum_fade": {
                    "lookback": 6,
                    "drop_pct": drop_pct / 100.0,  # PARAMETER SWEEP
                    "min_bars_in_pos": 4
                },
                "time_stop": {
                    "max_bars_1h": bars_max,  # PARAMETER SWEEP
                    "max_bars_4h": 8,
                    "max_bars_1d": 4
                }
            },
        ...
        result = subprocess.run([
            ...
            "--config", str(temp_backtest_path.absolute()),
            "--out", str(temp_dir_path / "results")
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                if json_line:
                    ...
                    print(f"✅ {result_data['trades']} trades, {result_data['win_rate']:.1%} win rate, {result_data['expectancy']:.0f} expectancy")
                    return result_data
                else:
                    print(f"❌ No JSON output found")
                    return {"status": "no_json", "bars_confirm": bars_confirm, "drop_pct": drop_pct, "bars_max": bars_max}
            except json.JSONDecodeError as e:
                print(f"❌ Failed to parse JSON: {e}")
                return {"status": "parse_error", "bars_confirm": bars_confirm, "drop_pct": drop_pct, "bars_max": bars_max}
        else:
            ...
            return {"status": "backtest_error", "bars_confirm": bars_confirm, "drop_pct": drop_pct, "bars_max": bars_max}

    except subprocess.TimeoutExpired:
        print(f"❌ Test timeout")
        return {"status": "timeout", "bars_confirm": bars_confirm, "drop_pct": drop_pct, "bars_max": bars_max}
...
def main():
    print("🔬 Exit Parameter Sweep - Final Version")
    print("=" * 50)
    ...
    print("-" * 90)
    print(f"{'Rank':<4} {'Confirm':<7} {'Drop%':<6} {'MaxBars':<8} {'Trades':<6} {'WinRate':<8} {'Expectancy':<10} {'MaxDD':<8}")
    print("-" * 90)

    for i, result in enumerate(successful_tests[:5]):
        print(f"{i+1:<4} {result['bars_confirm']:<7} {result['drop_pct']:<6.0f} {result['bars_max']:<8} "
    ...
    output_file = "exit_parameter_sweep_final_results.json"
