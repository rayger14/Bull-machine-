...
def simulate_weekly_trading_sequence():
    ...
    days = [
        {
            "day": "Monday",
            "market": "Strong uptrend continuation",
            "htf": "long",
            "mtf": "long",
            "ltf": "long",
            "nested": True,
    ...
    for day_data in days:
        ...
        htf = BiasCtx(
            tf="1D",
            bias=day_data["htf"],
            confirmed=day_data["htf"] != "neutral",
            strength=strength_map[day_data["htf"]],
            bars_confirmed=3 if day_data["htf"] != "neutral" else 0,
            ma_distance=0.04,
            trend_quality=0.75,
        )
        ...
        mtf = BiasCtx(
            tf="4H",
            bias=day_data["mtf"],
            confirmed=day_data["mtf"] != "neutral",
            strength=strength_map[day_data["mtf"]],
            bars_confirmed=2 if day_data["mtf"] != "neutral" else 0,
            ma_distance=0.025,
            trend_quality=0.68,
        )
    ...
    print(
        f"   Filtering Effectiveness: {(total_signals - total_trades) / total_signals * 100:.0f}% filtered out"
        if total_signals > 0
        else "   Filtering Effectiveness: N/A"
    )
