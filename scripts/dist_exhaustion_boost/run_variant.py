"""Distribution-exhaustion 2-of-3 sizing-boost shim.

Hypothesis: when (tf4h_wyckoff_bearish_score >= 0.6) AND (range_position_20 < 0.40)
at trade entry, multiply position size by X. Designed to test the
'distribution_exhaustion' high-conviction long entry idea from commit 977e6bf,
minus the OI component (which isn't in the historical feature store).

NO production code, configs, or YAMLs modified — monkey-patches only.

Usage:
    python3 scripts/dist_exhaustion_boost/run_variant.py \
        --boost 1.25 \
        --bearish-min 0.6 \
        --range-pos-max 0.40 \
        --start-date 2023-01-01 \
        --end-date 2024-12-31 \
        --output-dir results/dist_exhaustion_boost/X_1.25/test_2023_2024
"""
...
def install_shim(boost_x: float, bearish_min: float, range_pos_max: float):
    """Monkey-patch _open_position to apply 2-of-3 distribution_exhaustion boost."""
    state = {
        ...
        'feature_lookup': None,  # populated lazily from engine.features_df
    }

    original_open = StandaloneBacktestEngine._open_position

    def patched_open(self, *args, **kwargs):
        # Lazily wire features_df once
        if state['feature_lookup'] is None and hasattr(self, 'features_df'):
            df = self.features_df
            if 'tf4h_wyckoff_bearish_score' in df.columns and 'range_position_20' in df.columns:
                state['feature_lookup'] = df[['tf4h_wyckoff_bearish_score', 'range_position_20']]
                logger.info(f"[BOOST_SHIM] Feature lookup ready: {len(df):,} rows")
            else:
                missing = [c for c in ['tf4h_wyckoff_bearish_score', 'range_position_20'] if c not in df.columns]
                raise RuntimeError(f"Required features missing from parquet: {missing}")

        # Apply boost to long entries only (this is a long-bias signal)
        ...
        if direction == 'long' and timestamp is not None and state['feature_lookup'] is not None:
            ...
            if bearish >= state['bearish_min'] and range_pos < state['range_pos_max']:
                ...
                if 'allocated_size_pct' in kwargs:
                    ...
                    # Positional: figure out position of allocated_size_pct in sig
                    # Fall back to no-op if we can't find it cleanly
                    pass
                logger.debug(
                    f"[DIST_EX_BOOST] {timestamp} long entry: "
                    f"bearish={bearish:.3f}, range_pos={range_pos:.3f} → "
                    f"size × {state['boost_x']:.2f}"
                )
...
def main():
    ...
    state = install_shim(args.boost, args.bearish_min, args.range_pos_max)

    print('=' * 80)
    print('DISTRIBUTION-EXHAUSTION (2-of-3) SIZING BOOST')
    print('=' * 80)
    print(f'Boost X:                 {args.boost:.2f}')
    print(f'bearish_min threshold:   {args.bearish_min:.2f}')
    print(f'range_pos_max threshold: {args.range_pos_max:.2f}')
    ...
    print()
    print('=' * 80)
    print('DIST-EXHAUSTION BOOST TELEMETRY')
    print('=' * 80)
    ...
    if state['n_eligible_entries'] > 0:
        ...
        print(f'  → {pct:.1f}% of eligible entries had 2-of-3 condition met')
