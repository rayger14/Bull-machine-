...
BASE_CONFIG.update(
    {
        ...
        "portfolio": {"starting_cash": 100000, "exposure_cap_pct": 0.60, "max_positions": 8},
...
GRID = [
    # Conservative baseline
    {
        ...
        "momentum_fade.drop_pct": 0.45,
        "time_stop.bars_max": 72,
    },
    # Aggressive early exits
    {
        "name": "aggressive",
        "choch_against.bars_confirm": 1,
        "momentum_fade.drop_pct": 0.25,
        "time_stop.bars_max": 48,
    },
    # Moderate balanced
    {
        "name": "balanced",
        "choch_against.bars_confirm": 2,
        "momentum_fade.drop_pct": 0.35,
        "time_stop.bars_max": 72,
    },
    # Fast CHoCH, slow momentum
    {
        ...
        "choch_against.bars_confirm": 1,
        "momentum_fade.drop_pct": 0.45,
        "time_stop.bars_max": 96,
    },
    # Slow CHoCH, fast momentum
    {
        "name": "fast_momentum",
        "choch_against.bars_confirm": 3,
        "momentum_fade.drop_pct": 0.25,
        "time_stop.bars_max": 48,
    },
]
...
def main():
    ...
    for i, params in enumerate(GRID, 1):
        ...
        print(
            f"🧪 Run {i}/{total_tests} ({params['name']}): bars_confirm={params['choch_against.bars_confirm']}, "
    ...
    print("-" * 100)
    print(
        f"{'Rank':<4} {'Name':<12} {'Bars':<5} {'Drop%':<6} {'TimeMax':<7} {'Trades':<6} {'WinRate':<8} "
    ...
    for i, result in enumerate(successful_tests):
        print(
            f"{i + 1:<4} {result['name']:<12} {result['choch_against.bars_confirm']:<5} "
    ...
    if active_exits:
        ...
        print(
            f"   Performance: {best_active['trades']} trades, {best_active['win_rate']:.1%} win rate, "
            f"{best_active['expectancy']:.0f} expectancy"
        )
        print(
            f"   Exit breakdown: CHoCH={best_active['exits_choch']}, "
