...
def main():
        ...
        for max_risk_per_trade in max_risk_values:
            for exposure_cap_pct in exposure_cap_values:
                ...
                if result.get("status") == "success":
                    print(
                        f"✅ {result['trades']} trades, {result['win_rate']:.1%} win rate, {result['expectancy']:.0f} expectancy, {result['sharpe']:.2f} sharpe"
                    )
                else:
    ...
    print("-" * 95)
    print(
        f"{'Rank':<4} {'BaseRisk':<9} {'MaxRisk':<8} {'ExposureCap':<11} {'Trades':<6} {'WinRate':<8} {'Expectancy':<10} {'Sharpe':<8}"
    )
    print("-" * 95)

    for i, result in enumerate(successful_tests[:5]):
        print(
            f"{i + 1:<4} {result['base_risk_pct']:<9.3f} {result['max_risk_per_trade']:<8.3f} {result['exposure_cap_pct']:<11.2f} "
