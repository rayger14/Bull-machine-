...
class FusionEngineV141:
        ...
        if phase in ("A", "C", "accumulation_C", "distribution_C"):
            ...
            if len(df) >= 20:
                ...
                if vol_ratio < 1.2 and wyckoff_score < 0.85:
                    logging.info(
                        f"Regime veto: low_vol_{phase}, vol_ratio={vol_ratio:.2f}, wyckoff={wyckoff_score:.2f}"
                    )
                    # Log for telemetry
                    self._log_telemetry(
                        "regime_veto",
                        {"phase": phase, "vol_ratio": vol_ratio, "wyckoff": wyckoff_score},
                    )
                    return False
    ...
    def fuse_scores(
        ...
        logging.debug(
            f"Fusion result: agg={aggregate:.3f}, weighted={weighted_score:.3f}, "
