...
class AdvancedExitEvaluator:
    ...
    def evaluate_exits(self, df: pd.DataFrame, trade_plan: Dict,
                      confluence_scores: Dict, bars_since_entry: int,
                      mtf_context: Dict = None) -> Dict:
        """
        ...
        for rule in self.rules:
            try:
                decision = rule.evaluate(
                    df, trade_plan, confluence_scores,
                    bars_since_entry, mtf_context
                )

                if decision:
                    # Log telemetry
                    self._log_telemetry(rule.name, decision, trade_plan,
                                      confluence_scores, bars_since_entry)

                    # Apply the decision
                    trade_plan = self._apply_decision(trade_plan, decision,
                                                     bars_since_entry)
    ...
    def _apply_decision(self, trade_plan: Dict, decision: ExitDecision,
                       bars_since_entry: int) -> Dict:
    ...
    def _log_telemetry(self, rule_name: str, decision: ExitDecision,
                       trade_plan: Dict, scores: Dict, bars: int):
