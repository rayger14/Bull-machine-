...
class MarkupSOWUTWarning(BaseExitRule):
    ...
    def evaluate(
        ...
        veto_count = sum([premium_exhausted, vol_divergence, bojan_high, mtf_desync, wyckoff_weak])
...
class MarkupUTADRejection(BaseExitRule):
    ...
    def evaluate(
        ...
        false_breakout = current["high"] > prior_high and current["close"] < (
            current["low"] + (current["high"] - current["low"]) * self.cfg["wick_close_frac"]
        )
...
class MarkupExhaustion(BaseExitRule):
    ...
    required_keys = {"min_bars_since_entry", "retest_frac", "wyckoff_drop_floor", "aggregate_floor"}
...
class BojanExtremeProtection(BaseExitRule):
    ...
    required_keys = {"wick_atr_mult", "vol_under_sma_mult", "exit_pct", "require_htf_alignment"}
                ...
                if mtf_context and mtf_context.get("htf_resistance_near"):
                    return ExitDecision(
                        ...
                        metadata={
                            ...
                            "volume_ratio": current["volume"] / vol_sma,
                        },
                    )
        else:
            # Check for Bojan Low
...
class AbsorptionVsDistributionExit(BaseExitRule):
    ...
    def evaluate(
        ...
        if (
            volume_ratio >= self.cfg["high_vol_threshold"]
            and current["close"] < prior_low
            and displacement < self.cfg["close_through_pct"]
        ):
            return ExitDecision(
                action="partial",
                size_pct=self.cfg["partial_exit_pct"],
