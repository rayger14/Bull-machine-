...
class CHoCHAgainstDetector:
    ...
    def evaluate(
        ...
        for tf in ["4H", "1D"]:
            ...
            signal = self._check_choch_on_timeframe(symbol, position_bias, df, current_bar, tf)
    ...
    def _detect_bearish_choch(
        self, symbol: str, df: pd.DataFrame, current_bar: pd.Timestamp, timeframe: str
    ) -> Optional[ExitSignal]:
            ...
            if break_strength >= self.min_break_strength:
                ...
                if volume_confirmed:
                    ...
                    return ExitSignal(
                        ...
                        urgency=urgency,
                        reasons=[
                            f"Bearish CHoCH on {timeframe}",
                            f"Break strength: {break_strength:.2f}",
                        ],
                        context={
...
class TimeStopEvaluator:
    ...
    def evaluate(self, symbol: str, position_data: Dict[str, Any], current_bar: pd.Timestamp) -> Optional[ExitSignal]:
        ...
        logging.info(
            f"[TIME_STOP_DEBUG] sym={symbol} bar={current_bar} "
