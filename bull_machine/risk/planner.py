...
def _build_execution_rules(risk_cfg: Dict) -> Dict:
    return {'be_at':'tp1','trail_at':'tp2','trail_mode':risk_cfg.get('trail_mode','swing'),'ttl_bars':risk_cfg.get('ttl_bars',18)}
...
def _get_default_risk_plan(series: Series, signal: Signal) -> RiskPlan:
    ...
    return RiskPlan(entry=px, stop=stop, size=0.0, tp_levels=[], rules={'be_at':'tp1','trail_at':'tp2','trail_mode':'swing'})
