...
def combine(
    ...
    try:
        ...
        weights = signals_cfg.get(
            "weights",
            {"wyckoff": 0.60, "liquidity": 0.40, "smt": 0.0, "macro": 0.0, "temporal": 0.0},
        )
...
def _determine_signal_side(w: WyckoffResult, l: LiquidityResult) -> str:
    if w.bias == "long" and l.pressure in ["bullish", "neutral"]:
        return "long"
    if w.bias == "short" and l.pressure in ["bearish", "neutral"]:
        return "short"
    return "neutral"
...
def _build_signal_reasons(w: WyckoffResult, l: LiquidityResult, conf: float) -> List[str]:
    ...
    if l.fvgs:
        reasons.append(f"{len(l.fvgs)} FVGs detected")
    if l.order_blocks:
        reasons.append(f"{len(l.order_blocks)} Order Blocks detected")
    return reasons
