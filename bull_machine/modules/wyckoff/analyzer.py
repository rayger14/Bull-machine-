...
def analyze(series: Series, cfg: dict, state: dict) -> WyckoffResult:
    ...
    try:
        ...
        return WyckoffResult(
            regime=regime, phase=phase, bias=current_bias,
            phase_confidence=phase_confidence, trend_confidence=trend_confidence,
            range=range_data
        )
...
def _build_range_model(series: Series, lookback: int) -> Optional[Dict]:
    ...
    return {'low':range_low,'high':range_high,'mid':range_mid,'height':height,'within_range':within,'penetration':penetration}
...
def _calculate_bias_with_hysteresis(series: Series, state: dict, hysteresis_bars: int) -> Tuple[str, float]:
    ...
    if avg_change > 0.001: new_bias = 'long'
    elif avg_change < -0.001: new_bias = 'short'
    else: new_bias = 'neutral'
...
def _detect_wyckoff_phase(series: Series, bias: str, range_data: Optional[Dict]) -> Tuple[str, float]:
    ...
    changes = [abs(recent[i].close - recent[i-1].close)/recent[i-1].close for i in range(1, len(recent)) if recent[i-1].close]
    ...
    if range_data and range_data['within_range']:
        ...
        if bias == 'long': return 'E', 0.7
        elif bias == 'short': return 'E', 0.7
        else: return 'A', 0.5
...
def _determine_regime(phase: str, bias: str, range_data: Optional[Dict]) -> str:
    ...
    elif phase == 'E': return 'trending'
    elif range_data and range_data['within_range']: return 'ranging'
    else: return 'neutral'

def _get_neutral_wyckoff() -> WyckoffResult:
    return WyckoffResult(regime='neutral', phase='neutral', bias='neutral',
                         phase_confidence=0.0, trend_confidence=0.0, range=None)
