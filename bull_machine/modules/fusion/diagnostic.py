    ...
    def fuse_with_mtf(self, modules: Dict[str, Any], sync_report: Optional[Any] = None) -> Optional[Signal]:
        ...
        if wyckoff:
            ...
            layers.append({
                'name': 'wyckoff',
                'score': score,
                'side': getattr(wyckoff, 'bias', 'neutral')
            })
        ...
        for name, details in layer_details.items():
            logging.info(f"  {name}: raw={details['raw_score']:.3f} weight={details['weight']:.2f} weighted={details['weighted']:.3f} side={details['side']}")
            ...
            if triad_config.get('require', False):
                ...
                if override_config.get('enabled', False) and len(triad_scores) >= 2:
                    ...
                    if top_two_sum >= override_threshold:
                        logging.info(f"TRIAD_OVERRIDE: Exceptional confluence {top_two_sum:.3f} >= {override_threshold:.3f} - allowing signal")
