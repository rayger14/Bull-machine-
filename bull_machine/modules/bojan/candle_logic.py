...
def wick_magnets(df: pd.DataFrame, ob_level: float,
    ...
    try:
        ...
        touches = df[
            ((df['high'] >= ob_level * 0.999) & (df['low'] <= ob_level * 1.001))
        ]
        if len(touches) > 0:
    ...
    logging.debug(f"Bojan wick magnet: wick_ratio={wick_ratio:.2f}, "
...
def bojan_exit_signal(df: pd.DataFrame, position_bias: str,
                      htf_context: Dict = None) -> Optional[Dict]:
    """
