...
def strategy_from_df(
    ...
    try:
        ...
        print(
            f"🔥 FUSION_CONFIG: Using enhanced fusion with quality floors: {fusion_config.get('quality_floors', 'NONE')}"
        )
        ...
        engine_signal = {
            ...
            "metadata": {
                ...
                "subscores": {
                    ...
                    "momentum": momentum_result.get("score", 0.0)
                    if isinstance(momentum_result, dict)
                    else getattr(momentum_result, "score", 0.0)
                    if momentum_result
                    else 0.0,
                    "volume": volume_result.get("score", 0.0)
                    if isinstance(volume_result, dict)
                    else getattr(volume_result, "score", 0.0)
                    if volume_result
                    else 0.0,
                    "context": context_result.get("score", 0.0)
                    if isinstance(context_result, dict)
                    else getattr(context_result, "score", 0.0)
                    if context_result
                    else 0.0,
                },
