...
class BacktestEngine:
            ...
            for tf in tfs:
                ...
                for i in range(lookback, len(df_tf)):
                    ...
                    if action in ("long", "short"):
                        ...
                        can_add_result, reason = self.portfolio.can_add(
                            action, risk_plan, current_equity, base_risk_pct
                        )
                        ...
                        if can_add_result:
                            ...
                            logging.info(
                                f"PRE_TRADE symbol={sym} side={action} price={bar['close']:.4f} stop={stop_price:.4f} risk%={base_risk_pct:.4f}"
                            )
                            logging.info(
                                f"[ENGINE] Opening {action} position for {sym} @ {bar['close']:.4f}, size={signal.get('size', 1.0)}"
                            )
    ...
    def _evaluate_exit_signals(
        ...
        try:
            ...
            if exit_signal:
                logging.info(
                    f"[EXIT] {symbol}: {exit_signal.exit_type.value} "
