...
def compute_mock_layer_scores(df: pd.DataFrame, enabled_modules: List[str],
...
def simulate_trades(df: pd.DataFrame, fusion_scores: pd.DataFrame,
                   enter_threshold: float, config: Dict) -> List[Dict]:
    """
    ...
    for i, (idx, row) in enumerate(fusion_scores.iterrows()):
        ...
        if not in_position:
            ...
            if (bars_held >= 30 or  # Time stop
                pnl_pct >= 0.03 or  # 3% profit
                pnl_pct <= -0.015 or  # 1.5% loss
                row.get('global_veto', False)):  # Global veto
...
def run_ablation(df: pd.DataFrame, config: Dict) -> Dict:
    ...
    for layer_set in layer_sets:
        ...
        logging.info(f"  {set_name}: Sharpe={results[set_name]['sharpe']:.2f}, "
...
def run_backtest(df: pd.DataFrame, config: Dict, output_dir: str) -> Dict:
    ...
    telemetry_files = ['exits_applied.json', 'parameter_usage.json', 'layer_masks.json', 'exit_counts.json']
    for filename in telemetry_files:
        mock_data = {'mock': True, 'trades': len(trades), 'timestamp': pd.Timestamp.now().isoformat()}
