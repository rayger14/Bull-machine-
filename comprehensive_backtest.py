...
class ComprehensiveBacktester:
    ...
    def compute_layer_scores(self, df: pd.DataFrame, idx: int) -> Dict:
        ...
        scores["wyckoff"] = max(
            ...
            min(
                0.9,
                wyckoff_result["confidence"] * (1 if wyckoff_result["bias"] != "neutral" else 0.5),
            ),
        )
    ...
    def run_backtest(self, df: pd.DataFrame, symbol: str) -> Dict:
        ...
        print(
            f"   Results: {len(self.trades)} trades, {performance['win_rate']:.1f}% win rate, "
    ...
    def enter_position(
        ...
        risk_data = calculate_dynamic_position_size(base_risk, df.iloc[: idx + 1], liquidity_data)
...
def main():
    ...
    config = {
        ...
        "backtest": {"initial_balance": 10000, "commission": 0.0002, "slippage": 0.0003},
