            ...
            if result["action"] == "enter_trade":
                ...
                if signal and plan:
                    ...
                    if len(future_data) >= 1:
                        ...
                        print(
                            f"Trade #{len(trades)}: {entry_date} {side.upper()} @ ${entry_price:.0f} -> {outcome.upper()} @ ${exit_price:.0f} ({trade['pnl_pct']:.1f}%) [Conf: {signal.confidence:.3f}]"
                        )
...
def analyze_results(trades, version):
    ...
    if not trades:
        return {"version": version, "total_trades": 0, "win_rate": 0, "avg_pnl": 0, "sharpe": 0}
