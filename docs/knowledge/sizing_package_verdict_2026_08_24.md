...
## POST-CAP FIX (2026-08-27, pre-merge — caught by user question re live expression)
As first coded the dial multiplied risk BEFORE the margin cap: on cap-bound live positions (median entry notional exactly $52,500) 0.75x of an above-cap desired size still hit the cap — the dead-tape de-size would NEVER have expressed live. Fixed: dial applied post-cap in both paths. Re-validated: **$304,488 (+13.2%), PF 1.45, MaxDD −15.8%, 5/5 years; off-switch = baseline to the penny.** Caveats: Sharpe 1.43 (−0.10); 3,513 vs 3,512 trades (one marginal entry via margin feedback). Live-window static reweight (200 positions, corrected leg-sum notionals): actual $15,753 → $20,095 (+$4,342, 5/6 months better, June −47%); live is already cap-flat so the package's live payload is the dial. Prior +$89K reweight figure was an artifact of per-leg size accounting — documented as a trap.
