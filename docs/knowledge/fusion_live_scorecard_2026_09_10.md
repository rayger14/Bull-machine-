...
The reusable implementation passed task review and final integration review, including a scoped re-review of the final arithmetic/coverage fixes. It reproduces the independent raw-reference measurements below. The [registered contract](../superpowers/specs/2026-09-10-fusion-live-scorecard-design.md) fixes grouping, exclusions, metrics and limits before additional outcome rankings. This first descriptive unit is complete for local research; economic challengers remain unexecuted. No live or production changes.
...
Final review fixes at `26a0a5e` guard oversized numeric conversion, group arithmetic and cross-group aggregates. Unrepresentable groups are quarantined with all row indices; individually valid groups remain, with only unavailable summary fields explicitly null and named in `arithmetic_unavailable_metrics`. Representable means, medians and correlations remain available. The earliest retained partial exit is correctly reported as March 3 at 09:00 UTC, not 11:00 UTC. These numerical edge cases are not present in the frozen snapshot.

Final-wave regressions first produced 11 failures / 39 passes; focused GREEN: **50 passed**. Root's fresh full research suite after the final fix: **321 passed**, one existing urllib3/LibreSSL warning, 11.67 seconds. Scoped final re-review found both findings addressed and no new breakage. Production `engine/`, `bin/`, and `configs/` diff from phase base `4cfaded` is empty. All 17 archetype keys are present, including zero observed groups for volume_fade_chop and whipsaw; no definition was changed.

Private generated artifact at implementation `26a0a5e`: `results/research_validation_2026_09_10/fusion_scorecard/scorecard_20260910.json`, SHA256 `9c2aed6b00afc4eb52459e9ad5138f944c699293794dc5cbd2e8d098901cb903`. Policy source SHA256 `81e95fed279e320fcaccac372e30de0972a9ff7c977d501cfaabb945e341308a`. It embeds raw source hashes, snapshot clocks, all groups/exclusions/open inventory, all archetype summaries, signals, and check results. Raw source hashes matched; copied-input repeatability and input nonmutation passed; independently regrouped raw counts/subtotals/PF and SciPy rank correlations matched. Apart from the corrected first-exit bound, updated policy hash and new empty unavailable-metric lists, the report exactly matches its prior frozen output. Every source row is accounted for exactly once. Strict JSON serialization passes.
