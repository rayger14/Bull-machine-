...
- **Record-trade decomposition (2026-09-20, wick_trap +$4,968)** — attribution: tide×size 97.5% (+$123 entry-selection skill; ladder +$63 vs buy-hold; any prior-24h long won $4.2-5.6K). Boost stack = 36% of PnL; gap-through fills 11.7%. KEY STRUCTURAL INSIGHT: under fixed-notional sizing the 4.9-ATR stop's value (+$2,771 vs 2-ATR) is RUNG-SPACING not risk (MAE only 1.1 ATR) — a profit-target-spacing parameter masquerading as a risk parameter. Directional fix contributed $0 (bar passed old identity; NOTE upper wick 0.513 > lower 0.478 — a dominance-form gate would have KILLED the record trade; run that A/B knowing this). Fusion rejected the trade (0.24 vs 0.42, bypass) — Lesson #54 deepens. The narrative setup class (post-flush higher-low+coil+raid): 1-for-4 in 6.7mo — jackpot on an unproven class; this one trade = 45% of the loose class's 6-month PnL.
- **RED-TEAM CORRECTIONS (2026-09-22, Opus 5.5 review of the Sept autopsies)** — (1) **Squeeze-fuel timeline RETRACTED**: the Binance positioning series was read with a ~6h timestamp shift — gLS 0.94 net-short and the OI local max occurred ~5h AFTER the Sept-18 ignition, not before; gLS actually ROSE during the Sept-15 flush (retail dip-buying); ignition OI ROSE +2.8K by 15:00 (new positioning, NOT short liquidation — we own no liquidation data, never state it as fact). Squeeze-fuel study VETOED as declared: the z IS ls_ratio_extreme (corr 1.000, already gates funding_divergence = circular), ~26% price echo (z<-1 ≈ "rallied 3 days" = the buried tape-dial), price leg vacuous (99.6% conditional), fires 25% of hours; re-declaration must use the residualized event-onset form in the review or stay parked. (2) **Record-trade attribution downgraded**: prior-24h cohort has n_eff≈1-2 (P&L autocorr 0.93); sim error $538 > the $123 "skill" — single-trade attribution is NOT identifiable; whole-period baseline: random longs mean +$94, 50% WR, this trade = 99.2nd pct; it is >100% of wick_trap's live lifetime PnL (rest: −$3,775). (3) **Rung-spacing claim restated**: valid on this path only; stop width acts via survival + loss-size + spacing; the scalper X2 evidence is SURVIVAL (4h exit, no ladder) — do not cite it for spacing; all live wick_trap stops are ~5 ATR (constant). (4) **MACRO FACTS both autopsies missed**: the record entry bar (Sep 16 18:00 UTC) WAS the FOMC statement bar (25bp hike, first since 2023); ignition (Sep 18 13:30) was US cash open on quad-witching Friday + $433M ETF inflows. Action: tag FOMC/CPI/NFP bars in V23 and re-score event-bar entries. (5) 1-for-4 class = uninformative (post-hoc, n=4, Wilson CI 5-70%). (6) NEW FLAG: live oi_value (~$2.2B) ≠ Binance BTCUSDT OI (~$8.06B) — live OI source audit needed. Best next action per review: population-level wick_trap entry-skill test vs regime-matched random longs with event tags.
- **ENTRY-SKILL VERDICT: wick_trap FAILS the population test (2026-09-22, pre-registered)** — all 147 directional-era silo entries (2020→Aug 2026) replayed under a COMMON ladder engine (0.5/1/2R rungs 10/30/50%, 2.5-ATR trail after 1R, 120h cap, 4.94-ATR stop, $52.5K flat) vs 20 same-month random-hour longs each, seed 42, no re-rolls. Result: mean excess **−$19/trade** (median −$345), positive years 2/7 (only 2021 +$649 and 2023 +$30), bootstrap 90% CI [−$343, +$311] straddles zero. Bar was mean>0 AND ≥5/7 years AND CI excludes 0 → **CLEAN FAIL — wick_trap entries are statistically indistinguishable from regime-matched random longs**; the anti-selection/zero-median-alpha verdict now formally covers wick_trap's entry bars, not just LC's. Two secondary signals: (a) LOCAL timing exists — jittered ±6/12/24h entries earn $187 vs real $374, so the exact bar beats its own neighborhood by ~$187/trade, but the neighborhood is BELOW month-random ($393): wick_trap fires in locally weak windows and times the dip well, yet gains nothing vs just being long that month. (b) Event bars: 6/147 entries within FOMC/CPI/NFP dates (2022+ calendar), excess +$1,214 on-event vs −$72 off — n=6, hypothesis-only, feeds the event-tagging work. Implication: wick_trap's real PnL comes from month selection (regime exposure) + exit skew capture, NOT the entry trigger — consistent with the CTA identity; entry-side "improvements" to wick_trap should carry a strong null prior. Script `bin/studies/wick_trap_entry_skill_test.py`, per-trade results `docs/knowledge/wick_trap_entry_skill_results_2026_09_22.csv`.
- **LC is two setups under one name (2026-09-24, found autopsying the Sept-23 LC loss, −$1,346)** — the live RSI gate `rsi_extreme_65` accepts RSI >65 OR <35, so LC fires on both upside volume climaxes and downside flushes. LC-only silo on the LIVE dir (configs/champion/archetypes_v14rq/, V23 store, 2020→Aug 2026, n=123, +$41.5K): **RSI>65 side n=92, +$43.7K, 54% WR, 6/7 years positive = the entire edge**; **RSI<35 side n=31, −$2.2K, 35% WR, 1/6 years positive** (entries on a red bar in a falling tape; RSI<35 AND red entry bar n=29, −$4.3K, only 2024 positive). Candle colour / prior-6h direction / RSI side are almost the same split (92/31 each). The YAML description calls LC a "capitulation signal", but the money is in the breakout-climax side; the capitulation side is the leak. Recent live losers Sep 15 (RSI 29) and Sep 23 (RSI 31) are both on the losing side; Sep 11 (RSI 73) was a loss on the good side, and Sep 10 (RSI 26) won, so the live sample is mixed and small. STATUS: POST-HOC finding (found by looking at one loser); not shipped. Under the boosts-over-filters doctrine it is not a cut, because 2024 was positive. Next step is a pre-registered A/B (tilt the RSI>65 side vs exclude the RSI<35 side), judged on held-out data plus live accumulation. Sept-23 trade facts: entry 84,572 on the close of a −1.47% flush bar (vz 3.51, largest volume of the week) after 10h of lower highs from the 87,337 top; stop 82,878 = 2.7 ATR; best excursion +$260 (+0.15R, the 0.5R rung at 85,419 was never touched); stopped out Sep 24 09:00 in the next leg down; size $65,625 = 1.25× boosted, so the loss was ~25% larger than base.
