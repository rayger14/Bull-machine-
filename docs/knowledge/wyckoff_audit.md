...
### 2026-08-16 addendum 70 — FUNDING CARRY (BUILD 3/4): BTC perp funding carry is a REAL but THIN and PRICE-DOMINATED cash flow — the fade LOSES (A1+A2 FAIL); Form B underpowered. The founding derivatives-layer vision, finally measured on ~6yr, does not clear the bar on one asset after costs.

The founding vision always included a derivatives layer (oi_divergence archetype, funding_divergence, derivatives_heat CMI component) — parked since May-2026 for data history (Lesson #44: derivatives_heat needs 3+yr; disabled at 0% weight). That history now exists: data/cache/derivatives_hourly_full.parquet, binance_funding_rate = Binance REALIZED last_funding_rate (8h settlement) ffilled 1H, 2020-09 -> 2026-06 (~5.77yr). This build tests the purest derivatives thesis — CARRY — head-on. Branch study/funding-carry off study/tightened-entry, worktree-isolated. Files: idea_lab/funding_carry_PREREGISTRATION.txt (frozen + committed BEFORE any measurement, commit 3187431), run_funding_carry.py (Form A), run_funding_regime_dial.py (Form B). STUDY ONLY; production untouched. Parity 0.00%; causality 3-point 0/9 mismatches.

FUNDING MECHANICS (documented precisely — this is the whole point of a carry study). Perp funding is a PAID cash flow exchanged every 8h at 00/08/16 UTC: rate>0 => LONGS PAY SHORTS, rate<0 => SHORTS PAY LONGS, payment = rate*notional. To a position, cf = -side*rate*notional (short with rate>0 RECEIVES). BTC/Binance distribution (per-8h): mean 1.036e-4 (annualized x3x365 = ~11.3%), median 8.5e-5, 85th 1.0e-4, 99th 1.062e-3 (ann ~116%), min -1.19e-3 (ann ~-130%). CRITICAL SHAPE: the series is FLOOR-CLUSTERED at Binance's 0.01%/8h baseline — so the 85th percentile (~1.0e-4) sits essentially AT the baseline/median, ~11% annualized, NOT an extreme. This shape governs everything below. The carry literature framing (Koijen/Moskowitz/Pedersen/Vrugt "Carry" 2018; the crypto cash-and-carry basis trade): a carry return = the ex-ante yield you are paid for BEARING A RISK, and carry strategies are structurally SHORT-VOLATILITY — steady small premium, rare violent losses ("pennies in front of a steamroller"). The pre-registered honesty test was whether BTC funding carry earns its short-vol premium after costs, or is just disguised mean-reversion.

FORM A — FUNDING-EXTREME FADE (standalone carry; the primary deliverable). Frozen spec: on the 8h grid, f3 = trailing-3-day mean funding (9 settlements); expanding-causal 85/50/15th percentiles; SHORT when f3>P85 (collect the crowded-long funding), LONG when f3<P15; 1% risk, 3xATR(daily) wide stop; accrue funding each held 8h explicitly; exit on normalization (f3 back through P50) or stop; 5bps/side. RESULT (n=35 = 15 short/20 long, 51.6% time-in-market, 2020-09->2026-06):
  net -$2,714 | PF 0.83 | WR 46% | meanR -0.073 | MaxDD -6.6% (worst DD episode -6.65%)
  DECOMPOSITION: price_leg -$3,819 | funding_leg +$1,366 | costs -$261.
  funding_leg = only 10.4% of gross profit. annualized carry YIELD captured = 3.24% (NOT the ~11% headline — because you sit in-position only ~half the time and enter at near-baseline rates, and the winning years' PnL is price not funding).
  VERDICT vs pre-registered rule: A1 (net>0 & PF>=1.30) FAIL. A2 (funding >= 40% of gross profit) FAIL (10.4%). A3 (no squeeze episode < -15% book DD) technically PASS (worst -6.65%) but MOOT and structurally trivial — 1%-risk-per-trade sizing caps every stop at ~-1% book, so no SINGLE trade can breach -15%; the true risk is a CLUSTER of squeeze stops, which the -6.6% equity MaxDD already bounds. **Form A REJECTS.**

WHY IT FAILS (mechanism, decision-grade):
  (1) The funding leg IS genuinely positive (+$1,366) — collecting funding works directionally, confirming the mechanic. But it is TINY, because the 85th-percentile trigger fires at the ~11%-annualized baseline, not at a crowded extreme. The real squeeze crowding lives at the 99th pct (~116% ann), far above the trigger, and is essentially never reached by an 85th-pct rule on a floor-clustered series.
  (2) The price leg (-$3,819) DOMINATES and is negative: the fade repeatedly bets against price momentum and gets run over. Textbook short-vol.
  (3) The winning years are PRICE luck, not carry — the anti-carry signature: 2023 +$5,623 and 2025 +$1,184 (the only green years) had NEGATIVE funding legs (-$72, -$131); the carry cash flow was a drag those years and the wins were mean-reversion price moves. Conversely the years with the BIGGEST funding collection were the WORST net (2021 fund +$988 / net -$1,227; 2024 fund +$645 / net -$2,780). Funding-collected and net-PnL are INVERSELY related here — the exact opposite of a carry edge. This corroborates the live prior "funding_z inverted" (live_emergent_mining 2026-07-21, watch #5).

SQUEEZE-EPISODE TABLE (the death mode the spec demanded — 2021 bull squeezes where shorts-collecting-funding get run over). Worst price legs:
  2020-12-18 SHORT 8.7d: price -$1,002, funding +$51,  net -$958  (Dec-2020->Jan-2021 melt-up)
  2020-12-27 SHORT 5.7d: price -$992,   funding +$49,  net -$949  (same squeeze)
  2023-12-10 SHORT 30d:  price -$992,   funding +$209, net -$795  (long grind up)
  2024-02-27 SHORT 1.3d: price -$984,   funding +$14,  net -$982
  2024-11-12 SHORT 8.3d: price -$983,   funding +$33,  net -$959  (post-election rip)
  PER-YEAR death mode confirmed: 2021 shorts collected +$988 of funding but lost -$2,173 on price = net -$1,227 (pennies vs steamroller, exactly as pre-registered). 2022 was the mirror trap on the LONG side (all 5 trades lost, net -$4,391) — going long to collect NEGATIVE funding in a bear got the longs stopped as price kept falling. So BOTH sides of the fade have a regime death mode: shorts die in bull squeezes, longs die in bear slides — the fade is anti-correlated with the trend that the DOOR (add.48) is built to ride.

FORM B — FUNDING AS REGIME DIAL on the add.48 door (boost-shaped; pre-flagged underpowered). Frozen door run on BTC-USD daily; each entry flagged crowded-long (f3>expanding-P85) at entry; paired split, sizing-tier framing NEVER a gate. RESULT: BTC door fired n=9 (6 in the funding window) — clean n=4 meanR +0.578 PF 3.28, crowded n=2 meanR +0.712 PF 5.33; delta (clean-crowded) = -0.134 (crowded marginally BETTER, the OPPOSITE of the "crowded funding predicts worse door outcomes" hypothesis), bootstrap 95% CI [-1.90, +1.63] straddling 0. n<30/tier => **DIRECTIONAL ONLY, no adoption** (per escalation rule). Funding exists only for BTC and the daily door fires ~1.5x/yr on BTC, so Form B is STRUCTURALLY un-powerable on one asset — it would need a multi-asset perp-funding panel (ETH/SOL/etc.) to reach n>=30, which this build's on-disk data does not have. Escalated as such.

HONEST SHORT-VOL CHARACTERIZATION (the required framing): BTC funding carry on one asset after costs is a THIN, PRICE-DOMINATED short-vol trade. The steady premium (~11% annualized headline; ~3.2% actually captured by an 85th-pct fade after part-time exposure) is real but far too small to survive the price whipsaw of fading a trending, reflexive asset. The distribution's floor-cluster means the tradeable "extreme" defined at the 85th percentile is not extreme at all, and the genuine 99th-pct squeezes are precisely where the position gets run over. A carry book that is short-vol needs either (a) a genuinely extreme trigger with a hard tail-risk overlay, (b) a market-neutral basis structure (long spot / short perp = harvest funding with the price leg HEDGED — the real cash-and-carry, which this directional fade is NOT), or (c) cross-asset diversification to smooth the squeeze deaths. On a single directional BTC leg, it does not clear.

WHAT THIS DOES AND DOESN'T CLOSE. CLOSES: the directional funding-extreme fade as a standalone BTC edge (REJECT, decisively — the price leg dominates and the winning years are anti-carry). Consistent with the parked derivatives_heat (Lesson #44) and the live funding_z-inverted watch — the derivatives layer's founding promise does NOT survive first honest contact as a directional carry on one asset. DOESN'T TEST (explicit follow-ups): (1) MARKET-NEUTRAL basis carry (spot-perp cash-and-carry with the price leg hedged) — the theoretically-correct way to harvest funding, requires a spot leg not modeled here; the single most promising unexplored variant. (2) A CROSS-ASSET funding panel (ETH/SOL/major perps) — the only route to power Form B and to diversify the squeeze deaths; needs a multi-asset funding fetch. (3) A genuinely-extreme trigger (99th pct) WITH an explicit tail overlay — not tested (no grids; pre-registered 85th honored). None of these is deployable without fresh data and a fresh pre-registration; all are recorded as watch-items, none adopted.

WHAT AMENDS ONE_STRATEGY / FORWARD_TEST_SPEC / production: NOTHING. No form adopts. The frozen add.48 door, the 25-market forward basket (add.62), and the capped S&P sleeve (add.64) are untouched. derivatives_heat stays at 0% weight; no funding gate/boost is proposed for the door (Form B underpowered, point estimate anti-hypothesis). Standing orders honored: bypass_threshold untouched, no archetype disabled, no production config/code changed, recommendations-only. Nothing shipped; production untouched; worktree removed; branch study/funding-carry pushed. (Build 3 of 4; add.68 = momentum-short branch, add.69 reserved for build 2, this = 70.)
