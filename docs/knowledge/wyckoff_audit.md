...
### 2026-08-16 addendum 68 — THE MOMENTUM-SHORT DOOR (mirror of add.48, INVERTED): REJECT / watch-item. Pass rule 1/3 (A edge FAIL, B money PASS, C hedge FAIL). Geometry is faithful (parity 0.00%, 86% below-EMA200) but the breakdown-retest is STRUCTURALLY STARVED: T5 fires on only 8.9% of armed markdown bars — markets fall without retesting, so the mirror is NOT symmetric.

**Pre-registration** `idea_lab/momentum_short_PREREGISTRATION.txt` (committed c94e3b0 BEFORE measurement). Exact geometric mirror of the validated add.48 LONG trend-continuation door (`UnifiedArchetypeV2._m2`, m2_mode='broad'), inverted term-by-term: R0 (close<ema200 OR state!=BROKEN_UP), T1 eye_dir=='bear', T2 non-extension state, T3 bear CONFIRMED_BREAK → break_level=range_lower_1d, T4 close<=break_level (held below), T5 high>=break_level−0.5·ATR (pulled UP into retest zone), stop=created_high+0.25·ATR, struct TP1 at struct_range_low → BE → runner to downside measured move. NO new params (all imported from the long door). Daily exec, weekly N=5 struct range, price-only eye. Costs 2bps+3bps/side, risk 1%, $100k. `idea_lab/momentum_short_door.py` + `idea_lab/run_momentum_short.py`. 29 markets / 6 families; fresh Coinbase crypto dailies + yfinance equity-index/metals/energy/fx/single-stock. **Referee parity 0.00% on ALL 29 assets; 3-point causality preserved.**

**PASS RULE (pre-registered §6; all three required) → REJECT.**
- **(A) EDGE — FAIL.** Clean-shorting pool (crypto+equity-index+metal): n=183, **PF 0.98**, meanR −0.009, bootstrap 95% CI **[−0.376, +0.601]** (straddles 0). Need PF≥1.5 AND n≥50 AND CI-lo>0. The door has **no pooled short edge**. Family pooled: crypto PF 0.64 (n=27), equity-index PF 0.47 (n=80), metal PF 1.65 (n=76). The one "positive" clean family is a **single-asset SILVER artifact** (SILVER n=24 PF 3.69 +$37,985; GOLD 0.22, COPPER 0.42, PLATINUM 0.80) — drop SILVER and metals collapse. CPCV K-block (K6,m2): only **33% of folds PF>1**.
- **(B) MONEY — PASS (but hollow).** Pooled bear-window shorts (crypto CRYPTO_BEARS + equity-index EQUITY_BEARS): n=30, **PF 2.56**, +$11,219, meanR +0.374 (>1.2). The door DOES make money *when it manages to enter* in a bear — but n=30 over ~30 years of pooled bears is exactly the starvation symptom below. (Metals excluded from the money pool — no pre-defined bear calendar in the harness; flagged.)
- **(C) HEDGE — FAIL.** Clean-family book MaxDD: long-only **−7.21%** → long+short **−8.43%** (DEEPER). Adding the short **deepened** drawdown and barely moved PnL ($135,774 → $134,170; shorts net ≈ −$1.6k). The short is not a hedge for this long book.

**T5 RETEST FIRE-RATE — the asymmetry acid test (pre-reg §6 required mechanism).** Causal, dedup-free pass: among "armed" bars (regime_ok AND T1–T4: a real prior bear break, held below), how often does T5 (the up-retest) also fire? **Pooled bears: 305 / 3417 armed = 8.9%.** Worst in the fastest, most one-directional markdowns — crypto-2022 **4.0%**, eqidx-2022 **2.9%**, COVID 7.1%; slower grinds higher — dotcom 9.9%, GFC 9.8%, crypto-2025/26 10.3%. **This confirms the pre-flagged asymmetry:** the long door's breakout-retest generalizes because uptrends routinely pull back to retest broken resistance; the short mirror's breakdown-retest does NOT, because markdowns fall faster and shallower/rarer retest — price holds below the break and keeps going, so the entry is starved precisely in the windows the door exists to harvest. **This is a real feature of price, not a code bug** — the inversion is faithful (parity 0.00%; below-EMA200 share 86% = genuine breakdown-CONTINUATION short, correct DNA, NOT top-catching; the acid test the long-side campaign kept failing, this door passes).

**Refute control tell.** fx (add.62 REFUTES the long door on fx) is *profitable* short here (PF 1.59; EURUSD 2.63, USDJPY 2.14) while the clean families are not — a control-family "win" is evidence the apparent signal is noise/regime, reinforcing REJECT rather than undermining it.

**Convergence note (cross-reference, not my measurement).** The coordinator-relayed cascade-reversion build (recorded separately as add.71 with an explicit provenance note; I did not run it and record no cascade numbers here) independently concluded the lever is LEVEL-ANCHORED entries, not the detection source. Our T5 starvation points the same way: a *retest-gated* short entry is structurally starved; if the short is ever revisited the lever is a **level-anchored (not retest-gated) short entry**, or accepting the extension/continuation tier — NOT more sensor/detector work. The mirror question ("does the validated long door invert?") is now answered: **geometrically yes, economically no** — the retest premise is directionally asymmetric.

**Anti-overfit / honesty caveats.** Per-asset n is tiny (SOL/XRP n=0 — no setups; most crypto n≤5); the (B) pass rests on n=30; (A)'s only bright family is one asset (SILVER). Nothing here is deployable. STANDING ORDERS honored: nothing shipped, no production code/config touched, all 16 archetypes and live paper untouched, bypass_threshold untouched. Data caches (Coinbase crypto + yfinance wide basket) deleted after the run per lean-storage; worktree removed. add-on / new-door base rate: the momentum-short mirror joins the REJECT column.
