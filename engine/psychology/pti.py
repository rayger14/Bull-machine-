...
class PTISignal:
    ...
    Attributes:
        ...
        trap_type: 'spring' | 'utad' | 'bull_trap' | 'bear_trap' | 'none'
            - 'spring': Bearish trap (retail short) → bullish reversal
            - 'utad': Bullish trap (retail long) → bearish reversal
            - 'bull_trap'/'bear_trap': Alternative names (legacy)
            - 'none': No clear trap detected
...
def detect_rsi_divergence(df: pd.DataFrame, lookback: int = 20) -> Dict:
    ...
    if len(prev_highs) > 0:
        ...
        if price_new_high > prev_high * 1.01 and rsi_at_new_high < rsi_at_prev_high - 10:
            # Bearish divergence detected (increased from 5 to 10 points for statistical significance)
    ...
    if len(prev_lows) > 0:
        ...
        if price_new_low < prev_low * 0.99 and rsi_at_new_low > rsi_at_prev_low + 10:
            # Bullish divergence detected (increased from 5 to 10 points for statistical significance)
...
def calculate_pti(df: pd.DataFrame, timeframe: str = '4H',
    ...
    Trap Types:
        - 'utad' (UTAD): Retail trapped long at top (likely reversal down)
        - 'spring': Retail trapped short at bottom (likely reversal up)
    ...
    Example:
        ...
        >>> if pti.trap_type == 'utad' and pti.pti_score > 0.7:
        >>>     # Strong UTAD (bullish trap) → avoid longs, consider shorts
        >>>     fusion_score -= 0.15
    """
    config = config or {}
    ...
    # Determine trap type
    # FIXED: Use Wyckoff-style names expected by archetypes
    # - Bullish trap (retail long) = UTAD (Upthrust After Distribution) → reversal down
    # - Bearish trap (retail short) = Spring → reversal up
    if bullish_trap_signals >= 2:
        trap_type = 'utad'  # Retail trapped long → bearish reversal
    elif bearish_trap_signals >= 2:
        trap_type = 'spring'  # Retail trapped short → bullish reversal
    else:
        trap_type = 'none'
...
def apply_pti_fusion_adjustment(fusion_score: float, pti_signal: PTISignal,
    ...
    if pti_signal.trap_type == 'utad':
        if direction == 'long':
            # Trying to go long when retail trapped long = BAD
            adjustment = -0.15
            reasons.append(f"PTI UTAD ({pti_signal.pti_score:.2f}) - avoid longs")
        elif direction == 'short':
            # Shorting when retail trapped long = GOOD (fade the herd)
            adjustment = +0.05
            reasons.append(f"PTI UTAD ({pti_signal.pti_score:.2f}) - fade the herd")

    elif pti_signal.trap_type == 'spring':
        if direction == 'short':
            # Trying to go short when retail trapped short = BAD
            adjustment = -0.15
            reasons.append(f"PTI spring ({pti_signal.pti_score:.2f}) - avoid shorts")
        elif direction == 'long':
            # Going long when retail trapped short = GOOD
            adjustment = +0.05
            reasons.append(f"PTI spring ({pti_signal.pti_score:.2f}) - fade the herd")
