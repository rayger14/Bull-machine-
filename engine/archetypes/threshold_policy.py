...
class ThresholdPolicy:
    ...
    def _blend_regime_gates(self, regime_probs: Dict[str, float]) -> Dict[str, float]:
        ...
        for profile in self.regime_profiles.values():
            # Skip comment/metadata keys (start with underscore)
            gate_keys.update(k for k in profile.keys() if not k.startswith('_'))
    ...
    def get_all_thresholds(self, regime_label: str) -> Dict[str, Dict[str, float]]:
        """
        Get thresholds for all archetypes in a specific regime.

        This is a convenience method used by BullMachineStrategy to get
        thresholds for RuntimeContext construction.

        Args:
            regime_label: Regime label (e.g., 'risk_on', 'crisis')

        Returns:
            Dict mapping archetype_name -> {param: threshold}

        Example:
            >>> policy.get_all_thresholds('risk_on')
            {
                'spring': {'fusion': 0.35, 'liquidity': 0.14},
                'order_block_retest': {'fusion': 0.33, 'liquidity': 0.12},
                ...
            }
        """
        # Use regime probabilities with 100% weight on specified regime
        regime_probs = {regime_label: 1.0}

        # Use the full resolve pipeline to get thresholds
        return self.resolve(regime_probs, regime_label)
