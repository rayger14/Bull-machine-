...
ARCHETYPE_REGIMES = {
    ...
    "spring": ["all"],  # A: Wyckoff Spring (counter-trend reversal works in all regimes)
    "order_block_retest": ["all"],  # B: Order Block Retest (SMC order blocks work in all market conditions)
    "wick_trap": ["all"],  # C: Wick Trap Reversal (works in all regimes - counter-trend pattern)
...
class ArchetypeLogic:
    ...
    def __init__(self, config: dict):
        ...
        # ===================================================================
        # DOMAIN ENGINES: Wyckoff & SMC (Pre-computed Features)
        # ===================================================================
        # Domain engines are currently using pre-computed features from the feature store
        # Future: May initialize engines for real-time computation
        from engine import feature_flags as features
        self.wyckoff_domain_enabled = features.ENABLE_WYCKOFF
        self.smc_domain_enabled = features.ENABLE_SMC

        logger.info(f"[ArchetypeLogic] Domain engines: Wyckoff={self.wyckoff_domain_enabled}, SMC={self.smc_domain_enabled}")
        ...
        self.alias = {
            ...
            "adx": ["adx", "adx_14", "tf1h_adx_14"],  # Fixed: feature store has 'adx' not 'adx_14'
            "atr": ["atr_20", "atr_14", "atr", "tf1h_atr_20", "tf1h_atr_14"],
            "atr_percentile": ["atr_percentile", "tf1h_atr_percentile"],
            "vol_z": ["volume_zscore", "tf1h_volume_zscore"],
            # Candlestick patterns
            "wick_lower_ratio": ["wick_ratio", "lower_wick_ratio"],  # Fixed: feature store has 'wick_ratio' not 'wick_lower_ratio'
            # Extras (optional)
    ...
    def _apply_wyckoff_phase_boost(
        self, row: pd.Series, fusion: float
    ) -> Tuple[float, dict]:
        """
        Apply Wyckoff phase multiplier to fusion score.

        Reads pre-computed phase from feature store and applies phase-based multiplier (0.70-1.30).

        Args:
            row: Feature row
            fusion: Base fusion score

        Returns:
            (boosted_fusion, metadata)

        Metadata:
            - phase: Wyckoff phase name
            - multiplier: Applied multiplier
            - confidence: Phase detection confidence
            - original_fusion: Pre-boost fusion
        """
        try:
            # Read pre-computed phase from feature store
            wyckoff_phase = row.get('wyckoff_phase_abc', 'neutral')
            # Use wyckoff_pti_score as proxy confidence if phase_confidence missing
            wyckoff_confidence = row.get('wyckoff_phase_confidence', row.get('wyckoff_pti_score', 0.5))

            # Map phase to multiplier
            phase_multipliers = {
                'spring': 1.30,
                'B': 1.20,  # Reaccumulation
                'C': 1.20,  # Last point of support
                'A': 1.15,  # Accumulation
                'neutral': 1.00,
                'distribution': 0.85,
                'upthrust': 0.70
            ...
            base_multiplier = phase_multipliers.get(wyckoff_phase, 1.0)

            # Scale by confidence
            confidence_factor = 0.6 + (wyckoff_confidence * 0.4)
            multiplier = 1.0 + ((base_multiplier - 1.0) * confidence_factor)

            boosted_fusion = fusion * multiplier

            return boosted_fusion, {
                'phase': wyckoff_phase,
                'multiplier': multiplier,
                'confidence': wyckoff_confidence,
                'original_fusion': fusion,
                'boosted_fusion': boosted_fusion
            }

        except Exception as e:
            logger.error(f"Error in Wyckoff phase boost: {e}")
            return fusion, {'error': str(e), 'multiplier': 1.0}

    def _apply_smc_confluence_boost(
        self, row: pd.Series, fusion: float
    ) -> Tuple[float, dict]:
        """
        Apply SMC confluence multiplier to fusion score.

        Reads pre-computed SMC signals from feature store and applies confluence-based boost (0.90-1.20).

        Args:
            row: Feature row with SMC features
            fusion: Base fusion score

        Returns:
            (boosted_fusion, metadata)

        Required Features (pre-computed):
            - smc_ob_hit: Order Block proximity (0/1)
            - smc_fvg_hit: FVG proximity (0/1)
            - smc_sweep_recent: Recent sweep (0/1)
            - smc_bos_aligned: BOS in trend direction (0/1)
        """
        try:
            # Read SMC component hits from feature store (using actual column names)
            # Handle None values properly - convert to 0/1
            ob_hit = 1 if (row.get('smc_demand_zone', 0) or row.get('smc_supply_zone', 0)) else 0
            fvg_hit = 1 if (row.get('fvg_bullish', 0) or row.get('fvg_bearish', 0)) else 0
            sweep_hit = 1 if row.get('smc_liquidity_sweep', 0) else 0
            bos_hit = 1 if row.get('smc_bos', 0) else 0

            # Count aligned components
            components_aligned = ob_hit + fvg_hit + sweep_hit + bos_hit
            total_components = 4
            confluence_rate = components_aligned / total_components

            # Map to multiplier
            if confluence_rate >= 0.75:  # 3-4 components
                multiplier = 1.20
            elif confluence_rate >= 0.50:  # 2 components
                multiplier = 1.15
            elif confluence_rate >= 0.25:  # 1 component
                multiplier = 1.10
            else:  # 0 components
                multiplier = 0.90
            ...
            logger.error(f"Error in SMC confluence boost: {e}")
            return fusion, {'error': str(e), 'multiplier': 1.0}
    ...
    def _infer_direction(self, archetype_name: str) -> str:
        ...
        # E (Liquidity Compression) - LONG (bull archetype, conservative compression break)
        # Detects low volatility + narrow range consolidation, breaks bullish
        if archetype_name == "liquidity_compression":
            return "LONG"

        # S3 (Whipsaw), S8 (Volume Fade Chop) - EITHER (two-sided/ambiguous bear patterns)
        if archetype_name in ["whipsaw", "volume_fade_chop"]:
            return "EITHER"
    ...
    def detect(self, context: RuntimeContext) -> Tuple[Optional[str], float, float, Optional[str]]:
        ...
        **CRITICAL FIX #4 (2026-01-15)**: Now returns direction as 4th element to fix S5 short
        ...
        # =======================================================================
        # DOMAIN ENGINE LAYER - Apply boosts in sequence
        # =======================================================================

        # PR#6C: Wyckoff Event Vetos (EXISTING - Keep as-is)
        ...
        # DOMAIN 1: Wyckoff Phase Multipliers (NEW)
        if features.ENABLE_WYCKOFF:
            fusion_score, wyckoff_phase_meta = self._apply_wyckoff_phase_boost(
                context.row, fusion_score
            )
            if not hasattr(self, "_logged_wyckoff_phase"):
                logger.info(f"[WYCKOFF PHASE] First boost: {wyckoff_phase_meta}")
                self._logged_wyckoff_phase = True

        # DOMAIN 2: SMC Confluence Boosts (NEW)
        if features.ENABLE_SMC:
            fusion_score, smc_meta = self._apply_smc_confluence_boost(
                context.row, fusion_score
            )
            if not hasattr(self, "_logged_smc_boost"):
                logger.info(f"[SMC] First boost: {smc_meta}")
                self._logged_smc_boost = True

        # DOMAIN 3: Temporal Fusion Adjustment (EXISTING - Keep as-is)
    ...
    def _detect_all_archetypes(
        ...
        **CRITICAL FIX #4 (2026-01-15)**: Now returns direction to fix S5 short bug.
        ...
        archetype_map = {
            ...
            # Bear-biased archetypes (short-biased)
            "S1": ("liquidity_vacuum", self._check_S1, 12),  # Fixed: was "breakdown" (deprecated name)
        ...
        for letter, (name, check_func, priority) in archetype_map.items():
            ...
            # REGIME-AWARE ROUTING: Soft multiplier instead of hard gate
            # Hard gates were blocking ~70% of archetypes in crisis/risk_off
            # Now we apply a soft penalty (0.6-0.9) so signals can still fire if strong enough
            current_regime = context.regime_label if context else "neutral"
            ...
            if "all" not in allowed_regimes and current_regime not in allowed_regimes:
                # Soft penalty instead of hard skip
                # crisis/risk_off for bull archetypes: 0.6x (heavy penalty but not zero)
                # risk_on/neutral for bear archetypes: 0.7x (moderate penalty)
                if current_regime in ('crisis',):
                    regime_penalty = 0.6
                elif current_regime in ('risk_off',):
                    regime_penalty = 0.7
                else:
                    regime_penalty = 0.8
                logger.debug(
                    f"[REGIME SOFT] {name}: regime={current_regime} "
                    f"not in preferred={allowed_regimes}, penalty={regime_penalty}"
                )
            ...
            if isinstance(result, tuple):
                if len(result) == 4:
                    ...
                    if matched:
                        # Apply regime soft penalty to score
                        score *= regime_penalty
                        candidates.append((name, score, meta, priority, direction))
                        logger.debug(
                            f"[DISPATCH] {name} matched with score={score:.3f}, direction={direction}, regime_mult={regime_penalty}, meta={meta}"
                        )
                elif len(result) == 3:
                    ...
                    if matched:
                        ...
                        candidates.append((name, score, meta, priority, direction))
                        logger.debug(
                            f"[DISPATCH] {name} matched with score={score:.3f}, direction={direction} (inferred), regime_mult={regime_penalty}, meta={meta}"
                        )
            else:
                # Legacy bool return (not yet upgraded)
                if result:
                    direction = self._infer_direction(name)
                    adjusted_score = global_fusion_score * regime_penalty
                    candidates.append((name, adjusted_score, {}, priority, direction))
                    logger.debug(
                        f"[DISPATCH] {name} matched (legacy bool), using global_fusion={adjusted_score:.3f}, direction={direction}, regime_mult={regime_penalty}"
                    )
        ...
        if len(candidates) > 1:
            logger.info(
                f"[ROUTING DEBUG] context.regime_label={regime}, candidates={[(n, f'{s:.3f}') for n,s,_,_,_ in candidates]}"
            )
    ...
    def _detect_legacy_priority(
        ...
        **CRITICAL FIX #4 (2026-01-15)**: Now returns direction to fix S5 short bug.
        ...
        if self.enabled["E"] and _is_match(self._check_E(context)):
            return "liquidity_compression", fusion_score, liquidity_score, "LONG"
    ...
    def _check_A(self, context: RuntimeContext) -> tuple:
        ...
        **REFACTOR #2**: Standardized to return (matched, score, meta) tuple.
        ...
        """
        # ============================================================================
        # STEP 0: Regime confidence check (adaptive for probabilistic mode)
        # ============================================================================
        # Probabilistic regime (use_soft_controls=true) outputs probability distributions,
        # so confidence values are naturally lower (0.3-0.4 typical). The old binary regime
        # had higher confidence (0.5-0.9), so we adapt the threshold based on mode.

        # Check if using probabilistic regime (soft controls)
        has_regime_config = hasattr(context, 'regime_config')
        regime_config_value = context.regime_config if has_regime_config else None
        use_soft_controls = regime_config_value.get('use_soft_controls', False) if regime_config_value else False

        # DEBUG: Log first time to understand what's happening
        if not hasattr(self, '_logged_regime_mode'):
            logger.info(f"[REGIME MODE DEBUG] has_regime_config={has_regime_config}, regime_config={regime_config_value}, use_soft_controls={use_soft_controls}")
            self._logged_regime_mode = True

        # Adaptive threshold: 0.0 for probabilistic (no veto - use soft controls), 0.50 for binary
        # In probabilistic mode, confidence = 1 - instability, so low confidence is expected/normal
        MIN_REGIME_CONFIDENCE = 0.0 if use_soft_controls else 0.50
        ...
        if regime_confidence < MIN_REGIME_CONFIDENCE:
            return (
                ...
                0.0,
                {
                    ...
                    "explanation": f"Regime detection unreliable (mode: {'probabilistic' if use_soft_controls else 'binary'})",
                },
            )
        ...
        if wyckoff_spring_a:
            ...
            # Phase C filter: Only accumulation LPS (bullish)
            # Phase D LPS are distribution (bearish) - skip these
            wyckoff_phase = self.g(r, "wyckoff_phase_abc", "")
            if wyckoff_phase == "C":
                base_score = 0.40  # LPS + wick rejection combo
                detection_path = "wyckoff_lps_wick_phase_c"
            else:
                # Phase D LPS or other phases - skip
                return (False, 0.0, {
                    "reason": "lps_wrong_phase",
                    "phase": wyckoff_phase,
                    "explanation": "LPS must be in Phase C (accumulation), not Phase D (distribution)"
                })
        ...
        if use_wyckoff:
            ...
            # ADDITIONAL WYCKOFF PHASES: Reaccumulation (Phase B), Markup (Phase E)
            wyckoff_reaccumulation = self.g(context.row, "wyckoff_phase_abc", "") == "B"
            wyckoff_markup = self.g(context.row, "wyckoff_phase_abc", "") == "E"
            wyckoff_absorption = self.g(context.row, "wyckoff_absorption", False)

            if wyckoff_reaccumulation:
                domain_boost *= 1.50  # Phase B - reaccumulation before markup
                domain_signals.append("wyckoff_reaccumulation_phase")

            if wyckoff_markup:
                domain_boost *= 1.80  # Phase E - markup trend
                domain_signals.append("wyckoff_markup_phase")

            if wyckoff_absorption:
                domain_boost *= 0.80  # Range-bound caution - chop zone
    ...
    def _check_B(self, context: RuntimeContext) -> tuple:
        ...
        **CRITICAL FIX (2026-01-09)**: Disabled in RISK_ON regime due to catastrophic losses.
        ...
        # ============================================================================
        # PHASE 2: VOLUME FADE FILTER (Wyckoff principle: retest = lower volume)
        # ============================================================================
        # False breakouts typically have high volume as retail chases. True retests
        # have declining volume as smart money accumulates quietly.
        vol_z = self.g(context.row, "vol_z", 0.0)
        volume_fade_threshold = context.get_threshold("order_block_retest", "volume_fade_threshold", 0.0)

        if vol_z > volume_fade_threshold:  # Volume not fading enough
            # Allow exception during crisis (panic can sustain volume on valid retests)
            if not is_crisis:
                return (
                    False,
                    0.0,
                    {
                        "reason": "volume_not_fading",
                        "value": vol_z,
                        "threshold": volume_fade_threshold,
                        "explanation": "High volume retest indicates false breakout (retail chasing)",
                    },
                )

        # ============================================================================
        # PHASE 3: MULTI-TIMEFRAME CONFIRMATION (Avoid counter-trend trades)
        # ============================================================================
        # Order block retests are dangerous without higher timeframe confirmation.
        # Require either 4H BOS or strong momentum alignment to filter counter-trend.
        tf4h_bos_bullish = self.g(context.row, "tf4h_bos_bullish", False)
        # Fallback to fusion_momentum if momentum_alignment not available
        momentum_alignment = self.g(context.row, "momentum_alignment", None)
        if momentum_alignment is None:
            momentum_alignment = self.g(context.row, "fusion_momentum", 0.0)

        # MTF confirmation thresholds
        mtf_momentum_threshold = context.get_threshold("order_block_retest", "mtf_momentum_threshold", 0.6)
        mtf_wyckoff_exception = context.get_threshold("order_block_retest", "mtf_wyckoff_exception", 0.80)

        has_mtf_confirmation = tf4h_bos_bullish or momentum_alignment > mtf_momentum_threshold

        if not has_mtf_confirmation:
            # Allow exception if very strong Wyckoff signal (spring/accumulation)
            if wyckoff < mtf_wyckoff_exception:
                return (
                    False,
                    0.0,
                    {
                        "reason": "no_mtf_confirmation",
                        "tf4h_bos_bullish": tf4h_bos_bullish,
                        "momentum_alignment": momentum_alignment,
                        "wyckoff": wyckoff,
                        "explanation": "Counter-trend retest without HTF confirmation or strong Wyckoff",
                    },
                )

        # Archetype-specific scoring (not global fusion!)
        # PHASE 2 ENHANCEMENT: Include volume fade as quality factor
        # Negative vol_z is good (declining volume), convert to 0-1 scale
        volume_fade_score = max(0.0, min(1.0, (-vol_z + 2.0) / 3.0))  # Map [-1, 2] to [1, 0]

        # PHASE 3 ENHANCEMENT: Include MTF alignment as bonus
        mtf_bonus = 1.0
        if tf4h_bos_bullish:
            mtf_bonus = 1.3  # 30% bonus for 4H structure confirmation
        elif momentum_alignment > mtf_momentum_threshold:
            mtf_bonus = 1.15  # 15% bonus for momentum alignment
        ...
        # PHASE 2 ENHANCEMENT: Add volume_fade weight (reallocate from other components)
        if is_crisis:
            weights = context.get_threshold(
                "order_block_retest",
                "weights_crisis",
                {
                    "fusion": 0.30,  # Reduced from 0.35
                    "liquidity": 0.18,  # Reduced from 0.20
                    "momentum": 0.12,  # Reduced from 0.15
                    "wyckoff": 0.25,  # Increased from 0.20 (key accumulation detector in crisis)
                    "boms": 0.05,  # Reduced from 0.10 (optional bonus in crisis)
                    "volume_fade": 0.10,  # NEW: Volume quality is critical in crisis
                },
            )
        else:
            weights = context.get_threshold(
                ...
                "weights",
                {
                    ...
                    "wyckoff": 0.20,
                    "boms": 0.10,
                    "volume_fade": 0.10,  # NEW: Volume fade critical for filtering false breakouts
                },
            )
        ...
        # Apply MTF bonus (Phase 3 enhancement)
        base_score = base_score * mtf_bonus
        ...
        meta = {
            ...
            # Phase 2 & 3 metadata
            "vol_z": vol_z,
            "volume_fade_score": volume_fade_score,
            "tf4h_bos_bullish": tf4h_bos_bullish,
            "momentum_alignment": momentum_alignment,
            "mtf_bonus": mtf_bonus,
            "has_mtf_confirmation": has_mtf_confirmation,
    ...
    def _check_E(self, context: RuntimeContext) -> Tuple[bool, float, Dict]:
        ...
        return (
            ...
            "LONG",  # FIX: Changed from EITHER to LONG (E is bull archetype)
        )
    ...
    def _pattern_G(self, context: RuntimeContext):
        """Pattern detection for Archetype G: Re-Accumulation (Phase B)

        PHASE B IMPLEMENTATION: Detects Wyckoff Phase B re-accumulation patterns.
        Phase B represents the building of a cause during accumulation, where
        professional money is actively accumulating positions while creating
        sideways price action to trap retail traders.

        Key characteristics:
        - Wyckoff Phase B (re-accumulation)
        - BOMS strength confirming quality
        - Volume confirmation
        - Fusion gate for multi-domain validation
        """
        r = context.row

        # Primary signal: Wyckoff Phase B detection
        phase = self.g(r, "wyckoff_phase_abc", "neutral")
        if phase != "B":
            return None

        # Require BOMS strength for quality filter (indicates strong base building)
        boms_strength = self.g(r, "tf1d_boms_strength", 0.0)
        if boms_strength < 0.3:
            return None

        # Volume confirmation (require elevated volume during accumulation)
        volume_z = self.g(r, "volume_z", 0.0)
        if volume_z < 1.0:
            return None

        # Base score calculation
        # Higher BOMS strength = higher confidence in accumulation quality
        score = 0.40 + 0.25 * min(1.0, boms_strength / 0.6)
        tags = ["G", "re_accumulation", "phase_B", "LONG"]
        ...
        # Bonus for institutional order flow (BOMS very strong)
        if boms_strength > 0.5:
    ...
    def _check_G(self, context: RuntimeContext) -> Tuple[bool, float, Dict]:
        """Archetype G: Re-Accumulation (Phase B - LONG)"""
        ...
        # 4) Final fusion gate
        # NOTE: Keep "liquidity_sweep" key for backward compatibility with configs
        # This allows both "liquidity_sweep" and "re_accumulate" threshold keys to work
        fusion_th = context.get_threshold("re_accumulate", "fusion_threshold",
                                         context.get_threshold("liquidity_sweep", "fusion_threshold", 0.40))
    ...
    def _check_H(self, context: RuntimeContext) -> tuple:
        ...
        **DISPATCH FIX**: Returns (matched, score, meta) for true evaluate-all behavior.
        ...
        # ============================================================================
        # VERBOSE LOGGING: Gate checks
        # ============================================================================
        logger.info(f"[H] Gate checks - adx: {adx:.1f} (need >= {adx_th:.1f}), liquidity: {liq:.3f} (need < {liq_th:.3f})")

        # Gate checks
        if adx < adx_th:
            logger.info(f"[H] VETO: ADX too weak ({adx:.1f} < {adx_th:.1f})")
            return False, 0.0, {"reason": "adx_weak", "value": adx, "threshold": adx_th}
        if liq >= liq_th:  # Trap within trend needs LOW liquidity
            logger.info(f"[H] VETO: Liquidity too high ({liq:.3f} >= {liq_th:.3f})")
        ...
        # ============================================================================
        # VERBOSE LOGGING: Base fusion score calculation
        # ============================================================================
        logger.info(f"[H] Base fusion calculation - components: {components}")
        logger.info(f"[H] Base fusion calculation - weights: {weights}")
        logger.info(f"[H] Base score: {base_score:.3f}, archetype_weight: {archetype_weight:.3f}, initial_score: {score:.3f}")
        ...
        score_before_regime = score
        score = score * regime_penalty
        logger.info(f"[H] After regime penalty - regime: {current_regime}, penalty: {regime_penalty:.3f}, score: {score_before_regime:.3f} -> {score:.3f}")
        ...
        # ============================================================================
        # VERBOSE LOGGING: Domain boosts applied
        # ============================================================================
        logger.info(f"[H] After domain boosts - multiplier: {boost_multiplier:.3f}, score: {score_before_boost:.3f} -> {score:.3f}")
        logger.info(f"[H] Boost signals: {boost_signals}")
        logger.info(f"[H] Veto signals: {veto_signals}")
        logger.info(f"[H] Engines active: {engines_active}")
        ...
        # REGIME SOFT PENALTY (applied AFTER domain engines, BEFORE threshold gate)
        # ============================================================================
        score_before_regime_soft = score
        score, regime_penalty, regime_tags = self._apply_regime_soft_penalty(
            score, context, archetype_type="bull"
        )
        logger.info(f"[H] After regime soft penalty - penalty: {regime_penalty:.3f}, score: {score_before_regime_soft:.3f} -> {score:.3f}")
        ...
        logger.info(f"[H] After soft gating - passed: {passed_gating}, score: {raw_score_before_gating:.3f} -> {score:.3f}")
        logger.info(f"[H] Threshold gate check - score: {score:.3f}, fusion_th: {fusion_th:.3f}, pass: {score >= fusion_th}")

        if not passed_gating:
            logger.info(f"[H] VETO: Failed soft gating - {gating_metadata}")
            return (
                False,
                score,
                {
        ...
        # ============================================================================
        if score < fusion_th:
            logger.info(f"[H] VETO: score < threshold ({score:.3f} < {fusion_th:.3f})")
            return (
                False,
                score,
                {
        ...
        logger.info(f"[H] ✓ PASSED ALL GATES - Final score: {score:.3f}")
    ...
    def _pattern_K(self, context: RuntimeContext):
        ...
        # Read thresholds from config (NOT hardcoded!)
        wick_threshold = context.get_threshold("wick_trap", "wick_lower_threshold", 0.60)
        rsi_threshold = context.get_threshold("wick_trap", "rsi_threshold", 40)

        logger.info(f"[K] Pattern check - wick_lower_ratio: {wick_low:.3f} (need >= {wick_threshold:.2f}), rsi_14: {rsi:.1f} (need <= {rsi_threshold})")
        if wick_low >= wick_threshold and rsi <= rsi_threshold:
            score = 0.50 + 0.25 * wick_low
            logger.info(f"[K] Pattern matched! wick_low: {wick_low:.3f}, rsi: {rsi:.1f}, score: {score:.3f}")
            return True, score, ["K", "wick_trap", "LONG"]
        else:
            logger.info(f"[K] Pattern NOT matched - wick_low check: {wick_low >= wick_threshold}, rsi check: {rsi <= rsi_threshold}")
        return None
    ...
    def _check_K(self, context: RuntimeContext) -> Tuple[bool, float, Dict]:
        ...
        if regime_confidence < MIN_REGIME_CONFIDENCE:
            logger.info(f"[K] VETO: regime_confidence_too_low ({regime_confidence:.2f} < {MIN_REGIME_CONFIDENCE:.2f})")
            return (
                False,
                0.0,
                {
        ...
        matched, base_score, pattern_tags = base_result  # base_score in [0,1]
        logger.info(f"[K] Base pattern matched - base_score: {base_score:.3f}, tags: {pattern_tags}")

        # 2) Global safety vetoes (soft - use penalties not hard returns)
        # Note: Keep these soft - use penalties not hard returns

        # 3) Domain modifiers (standardized across all)
        score_before_domain = base_score
        score, domain_metadata = self._apply_domain_engines(context, base_score, pattern_tags)
        logger.info(f"[K] After domain engines - score: {score_before_domain:.3f} -> {score:.3f}")
        logger.info(f"[K] Domain metadata: {domain_metadata}")
        ...
        logger.info(f"[K] After regime soft penalty - penalty: {regime_penalty:.3f}, score: {score_before_regime:.3f} -> {score:.3f}")
        ...
        logger.info(f"[K] After soft gating - passed: {passed_gating}, score: {raw_score_before_gating:.3f} -> {score:.3f}")
        logger.info(f"[K] Threshold gate check - score: {score:.3f}, fusion_th: {fusion_th:.3f}, pass: {score >= fusion_th}")

        if not passed_gating:
            logger.info(f"[K] VETO: Failed soft gating - {gating_metadata}")
            return (
                False,
                score,
                {
        ...
        if score < fusion_th:
            logger.info(f"[K] VETO: score < threshold ({score:.3f} < {fusion_th:.3f})")
            return (
                False,
                score,
                {
        ...
        logger.info(f"[K] ✓ PASSED ALL GATES - Final score: {score:.3f}")
    ...
    def _pattern_M(self, context: RuntimeContext):
        ...
        FIXED LOGIC V3 (PROPER COIL DETECTION):
        - Uses sophisticated multi-component coil detection:
          * tf4h_coil_score: Combined ATR + BB squeeze metric (0-1, higher = tighter)
          * tf4h_coil_breakout: Binary flag when breaking out of compression
        - Validates with tf4h_bos_bullish (4H bullish BOS confirms breakout direction)
        - Requires sustained compression (high coil score) + breakout trigger

        Expected signals: 10-20 high-quality coil breakouts per year
        """
        r = context.row

        # ==============================================================================
        # COIL DETECTION: Multi-timeframe compression analysis
        # ==============================================================================
        # Use 4H coil features as primary (smoother, fewer false signals than 1H)
        coil_score_4h = self.g(r, "tf4h_coil_score", 0.0)
        is_coiling_4h = self.g(r, "tf4h_is_coiling", False)
        coil_breakout_4h = self.g(r, "tf4h_coil_breakout", False)

        # Get 1H coil score for additional context (faster detection)
        coil_score_1h = self.g(r, "tf1h_coil_score", 0.0)

        # Get 1D coil score for macro compression context
        coil_score_1d = self.g(r, "tf1d_coil_score", 0.0)

        # Configurable thresholds
        min_coil_score = context.get_threshold("coil_break", "min_coil_score", 0.55)
        require_breakout_flag = context.get_threshold("coil_break", "require_breakout_flag", True)

        # ==============================================================================
        # PATTERN MATCHING: Coil compression + breakout trigger
        # ==============================================================================
        # Option 1: Strict mode - require breakout flag (breaking out NOW)
        # Option 2: Relaxed mode - just high coil score + BOS (in compression state)

        has_breakout_trigger = coil_breakout_4h if require_breakout_flag else True
        has_high_compression = coil_score_4h >= min_coil_score
        has_bos_confirmation = self.g(r, "tf4h_bos_bullish", False)

        # Core pattern: Tight coil (4H) + breakout trigger + 4H BOS
        if has_high_compression and has_breakout_trigger and has_bos_confirmation:
            # ===========================================================================
            # SCORING: Reward stronger compression and multi-TF alignment
            # ===========================================================================
            # Base score: 0.45
            base_score = 0.45

            # Bonus 1: 4H coil intensity (0-0.15)
            # Higher coil score = tighter compression = stronger spring potential
            coil_intensity_bonus = 0.15 * min(1.0, (coil_score_4h - min_coil_score) / (1.0 - min_coil_score))

            # Bonus 2: Multi-timeframe coil alignment (0-0.10)
            # If 1H and 1D also show compression, it's a stronger setup
            mtf_alignment = 0.0
            if coil_score_1h >= min_coil_score:
                mtf_alignment += 0.05  # 1H also coiling
            if coil_score_1d >= min_coil_score:
                mtf_alignment += 0.05  # 1D also coiling

            # Total score (max: 0.70)
            score = base_score + coil_intensity_bonus + mtf_alignment
            score = min(0.70, score)  # Cap at 0.70

            # Metadata for debugging
            metadata_tags = ["M", "coil_break", "LONG"]

            return True, score, metadata_tags

        # ==============================================================================
        # FALLBACK: Legacy ATR-based detection (if coil features unavailable)
        # ==============================================================================
        # This ensures backward compatibility with older feature stores
        if coil_score_4h == 0.0:  # Coil features not calculated
            ...
            if atr_pct < atr_pct_max and self.g(r, "tf4h_bos_bullish", False):
                ...
                return True, score, ["M", "coil_break", "LONG", "fallback"]
    ...
    def _check_S1(self, context: RuntimeContext) -> Tuple[bool, float, Dict]:
        ...
        # ============================================================================
        # FIX (2026-01-22): DISABLED crisis routing because V1 fallback not implemented
        #
        # ORIGINAL INTENT: Route V2 to crisis-only environments, use V1 for normal markets
        # ACTUAL BEHAVIOR: When crisis_composite < threshold, V1 fallback returns False (not implemented)
        # RESULT: 0 trades in normal markets (Q1 2023: 97.9% of bars have crisis_composite=0)
        #
        # NEW APPROACH: Always use V2 logic, relax V2 hard gates via config
        # - crisis_routing_enabled=False (DEFAULT): Always use V2
        # - crisis_routing_enabled=True: Use old routing behavior
        # ============================================================================
        crisis_routing_enabled = context.get_threshold(
            "liquidity_vacuum", "crisis_routing_enabled", False
        )

        if use_v2_logic and crisis_routing_enabled:
            crisis_composite = self.g(context.row, "crisis_composite", 0.0)
        ...
        if use_v2_logic and has_v2_features:
            ...
            # Apply domain boost to final score
            score_before_boost = score  # BUGFIX: Initialize before boost is applied
            score = score * boost_multiplier
