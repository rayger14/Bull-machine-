...
def simulate_mtf_signals(data_dict, symbol):
    ...
    market_scenarios = [
        {'name': 'Strong Bull Trend', 'htf_bias': 'long', 'htf_strength': 0.85, 'mtf_bias': 'long', 'mtf_strength': 0.75, 'ltf_bias': 'long'},
        {'name': 'Bear Market', 'htf_bias': 'short', 'htf_strength': 0.80, 'mtf_bias': 'short', 'mtf_strength': 0.70, 'ltf_bias': 'short'},
        {'name': 'Choppy Sideways', 'htf_bias': 'neutral', 'htf_strength': 0.45, 'mtf_bias': 'neutral', 'mtf_strength': 0.40, 'ltf_bias': 'neutral'},
        {'name': 'HTF-LTF Conflict', 'htf_bias': 'long', 'htf_strength': 0.75, 'mtf_bias': 'long', 'mtf_strength': 0.65, 'ltf_bias': 'short'},
        {'name': 'Weak Alignment', 'htf_bias': 'long', 'htf_strength': 0.60, 'mtf_bias': 'neutral', 'mtf_strength': 0.50, 'ltf_bias': 'long'},
    ]
    ...
    for scenario in market_scenarios:
        ...
        htf = BiasCtx(
            tf="1D", bias=scenario['htf_bias'], confirmed=True,
            strength=scenario['htf_strength'], bars_confirmed=3,
            ma_distance=0.04, trend_quality=0.75
        )
        mtf = BiasCtx(
            tf="4H", bias=scenario['mtf_bias'], confirmed=True,
            strength=scenario['mtf_strength'], bars_confirmed=2,
            ma_distance=0.02, trend_quality=0.65
        )
...
def run_comprehensive_simulation():
    ...
    for symbol, files in datasets.items():
        ...
        print(f"     MTF Decisions: {mtf_decisions['allow']} ALLOW, {mtf_decisions['raise']} RAISE, {mtf_decisions['veto']} VETO")
    ...
    print(f"   Average per trade: {total_pnl/total_trades:+.2f}%" if total_trades > 0 else "   Average per trade: N/A")
    ...
    for i, signal in enumerate(top_signals, 1):
        ...
        print(f"      Scenario: {signal['scenario']} | MTF: {signal['mtf_decision']} | Alignment: {signal['alignment_score']:.1%}")
