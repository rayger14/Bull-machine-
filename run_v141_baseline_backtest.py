...
def simulate_trades_v141_baseline(
...
def main():
    ...
    for idx, row in layer_scores_df.iterrows():
        ...
        fusion_result = fusion_engine.fuse_scores(
            layer_dict, quality_floors=config.get("quality_floors", {}), df=df.loc[[idx]]
        )
    ...
    with open(results_dir / "baseline_results.json", "w") as f:
        json.dump(
            {"performance": performance, "sample_trades": trades[:10], "config": config},
            f,
            indent=2,
            default=str,
        )
