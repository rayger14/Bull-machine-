...
def test_v13_mtf_integration(csv_path, symbol, timeframe,
                           thresholds=[0.30, 0.35, 0.40],
                           test_interval=10, max_tests=50,
                           mtf_enabled=True):
    """
            ...
            try:
                ...
                if result['action'] == 'enter_trade':
                    ...
                    if signal and plan:
                        ...
                        print(f"  Trade #{len(trades)}: {signal.side.upper()} @ ${plan.entry:.2f} [Conf: {signal.confidence:.3f}]")
